...
// get transaksi by tgl
app.get('/byDate', async (req, res) => {
  const { date, id_user } = req.query;
  let result;
  const tgl_transaksi = new Date(date);

  if (date == '' || date == null) {
    result = await transaksi.findAll({
    ...
    result = await transaksi.findAll({
      where: { tgl_transaksi: tgl_transaksi, id_user: id_user },
      order: [
        ['id_transaksi', 'DESC'],
        ['tgl_transaksi', 'DESC'],
      ],
    ...
    .then((result) => {
      ...
      detail_transaksi
        ...
        .then((result) => {
          ...
          meja
            .update(dataMeja, { where: { id_meja: req.body.id_meja } })
            .then((result) => {
              res.json({
                message: 'Success',
              });
